...
//        }
    ...
    private void handleEarlyInteractiveChange() {
        synchronized (mLock) {
            if (mInteractive) {
                // Waking up...
                final int onwhy = translateOnReason(mInteractiveChangeReason);
                ...
                mHandler.post(new Runnable() {
                    ...
                    public void run() {
                        // BEGIN robert_lcc@asus.com for smart charging
                        if(onwhy != 0) {
                            EventLog.writeEvent(EventLogTags.POWER_SCREEN_STATE, onwhy, 0, 0, 0);
                        } else {
    ...
    private static int translateOffReason(int reason) {
        switch (reason) {
            ...
            case PowerManager.GO_TO_SLEEP_REASON_TIMEOUT_POSTPONED:
    ...
    // BEGIN robert_lcc@asus.com for smart charging
    private static int translateOnReason(int reason) {
        switch (reason) {
            case PowerManager.WAKE_UP_REASON_POWER_BUTTON:
            case PowerManager.WAKE_UP_REASON_POWER_IN_OUT:
            case PowerManager.WAKE_UP_REASON_COVER_IN_OUT:
            case PowerManager.WAKE_UP_REASON_GESTURE:
            case PowerManager.WAKE_UP_REASON_INSTANTCAMERA:
            case PowerManager.WAKE_UP_REASON_FINGERPRINT:
                return WindowManagerPolicy.OFF_BECAUSE_OF_USER;
            case PowerManager.WAKE_UP_REASON_WINDOWMANAGER:
                return WindowManagerPolicy.OFF_BECAUSE_OF_WM_TURN_ON;
            default:
                return WindowManagerPolicy.OFF_BECAUSE_OF_UNKNOWN;
        }
    }
    // END
    ...
    private void updatePendingBroadcastLocked() {
        ...
        if (!mBroadcastInProgress
           ...
           // BEGIN robert_lcc@asus.com for smart charging
           if (mPendingInteractiveState == INTERACTIVE_STATE_AWAKE) {
               int onwhy = translateOnReason(mInteractiveChangeReason);
               msg.arg1 = onwhy;
          } else if(mPendingInteractiveState == INTERACTIVE_STATE_ASLEEP){
                if(mInteractiveChangeReason == PowerManager.GO_TO_SLEEP_REASON_TIMEOUT_POSTPONED){
                    msg.arg1 = WindowManagerPolicy.OFF_BECAUSE_OF_TIMEOUT_POSTPONED;
                } else if(mInteractiveChangeReason == PowerManager.GO_TO_SLEEP_REASON_TIMEOUT){
                    msg.arg1 = WindowManagerPolicy.OFF_BECAUSE_OF_TIMEOUT;
                } else {
                    msg.arg1 = WindowManagerPolicy.OFF_BECAUSE_OF_UNKNOWN;
                }
           }
           // END
            msg.setAsynchronous(true);
    ...
    private void sendWakeUpBroadcast() {
        ...
        if (ActivityManagerNative.isSystemReady()) {
           ...
           // BEGIN robert_lcc@asus.com for smart charging
           mScreenOnIntent.putExtra("screen_chg_reason", mWakeChgReason);
           // END
    ...
    private void sendGoToSleepBroadcast() {
        ...
        if (ActivityManagerNative.isSystemReady()) {
           ...
           // BEGIN robert_lcc@asus.com for smart charging
           mScreenOffIntent.putExtra("screen_chg_reason", mWakeChgReason);
           // END
     ...
     * @hide
     */
    public int getLastGoToSleepReason() {
        int reason;
        synchronized (mLock) {
            reason = mInteractiveChangeReason;
        }
        return reason;
    }
